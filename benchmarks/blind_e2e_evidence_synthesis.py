...
class EvidenceSynthesisHarnessProvider(BaseProvider):
    def __init__(self):
        self._turn = 0

    @property
    def id(self) -> str:
        return "blind_e2e_evidence_synthesis_provider"

    @property
    def name(self) -> str:
        return "Blind E2E Evidence Synthesis Provider"
    ...
    async def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        if last_msg["role"] == "tool":
            return ChatResponse(
                content=(
                    "Evidence set: runtime session flow, end-to-end flight tests, and kernel lifecycle. "
                    "Observation: the runtime exposes staged execution and session lifecycle transitions. "
                    "Relevance: these are the most relevant contract boundaries in the current repository. "
                    "Confidence: moderate. "
                    "Conclusion: I still lack a concrete defect target with enough support to propose a minimal fix."
                )
            )
...
async def run_evidence_synthesis_harness(session_id: str | None = None, output_dir: Path | None = None) -> dict:
    timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    resolved_session_id = session_id or f"blind_e2e_synthesis_{timestamp}"
    ...
    await runtime.register_provider(EvidenceSynthesisHarnessProvider())
    runtime.tool_registry.register(ReadFileTool())

    ctx = SessionContext(
        project_path="/home/user/Projects/lisa",
        workspace_name="blind_e2e_evidence_synthesis",
        provider_id="blind_e2e_evidence_synthesis_provider",
    ...
    artifact = {
        "experiment_id": "BLIND-E2E-004",
        "title": "Evidence Synthesis Blind Flight",
...
if __name__ == "__main__":
    result = asyncio.run(run_evidence_synthesis_harness())
    print("Evidence synthesis harness completed.")
