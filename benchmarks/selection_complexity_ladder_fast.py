...
async def run_case(case: dict) -> dict:
    ...
    recorder = FlightRecorder(session_id=f"selection_ladder_fast_{case['id']}", log_dir=Path(__file__).resolve().parent)
    ...
    ctx = SessionContext(
        ...
        workspace_name="selection_ladder_fast",
...
async def main() -> None:
    ...
    artifact = {
        ...
        "title": "Selection Complexity Ladder (Fast)",
        "timestamp": datetime.now().isoformat(),
        "results": results,
    }
    out_path = Path(__file__).resolve().parent / f"selection_complexity_ladder_fast_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.json"
